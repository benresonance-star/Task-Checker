export const theme = {
  ...
  components: {
    ...
    profile: {
      card: 'flex items-center gap-3 px-3 py-2 bg-gray-100 dark:bg-black/40 rounded-metadata border border-gray-200 dark:border-gray-700 mb-2',
    ...
    modal: {
      ...
      infoBox: 'p-4 rounded-metadata border-2 mb-8',
      ...
      buttonPrimary: 'w-full h-14 rounded-button font-black uppercase tracking-widest shadow-lg text-white',
    ...
    dangerModal: {
      ...
      infoBox: 'p-6 bg-red-600/10 dark:bg-red-900/40 rounded-metadata border-2 border-red-600 mb-8',
      message: 'text-lg font-black text-red-600 dark:text-red-400 uppercase leading-tight',
      buttonPrimary: 'w-full h-16 rounded-button font-black uppercase tracking-widest bg-red-600 hover:bg-red-700 text-white shadow-2xl',
    ...
    dashboard: {
      projectInfoContainer: 'bg-[var(--project-info-bg)] rounded-project-info border border-[var(--project-info-border)] mb-8 shadow-sm overflow-hidden',
      header: 'flex flex-col sm:flex-row sm:items-center justify-between p-6 cursor-pointer hover:bg-black/5 dark:hover:bg-white/5 transition-colors gap-4',
      sectionCard: 'space-y-4 border border-[var(--metadata-card-border)] rounded-metadata p-6 shadow-sm bg-[var(--metadata-card-bg)]',
      ...
      consultantCard: 'bg-[var(--metadata-card-bg)] rounded-metadata border border-[var(--metadata-card-border)] p-6 shadow-sm hover:border-google-blue transition-all group/card',
      checklistContainer: 'bg-[var(--checklist-bg)] rounded-project-info p-4 md:p-12 border border-[var(--checklist-border)] shadow-sm',
    },
    planner: {
      pulseBg: 'bg-[var(--planner-pulse-bg)]',
      pulseBorder: 'border-[var(--planner-pulse-border)]',
      pulseText: 'text-[var(--planner-pulse-text)]',
      pulseAlertIcon: 'text-[var(--planner-pulse-alert-icon)]',
      sectionTitle: 'text-[var(--planner-section-title)]',
      tokenActiveBg: 'bg-[var(--planner-token-active-bg)]',
      tokenActiveText: 'text-[var(--planner-token-active-text)]',
      tokenInactiveBg: 'bg-[var(--planner-token-inactive-bg)]',
      tokenInactiveBorder: 'border-[var(--planner-token-inactive-border)]',
      tokenInactiveText: 'text-[var(--planner-token-inactive-text)]',
      tokenIcon: 'text-[var(--planner-token-icon)]',
      spotlightBg: 'bg-[var(--planner-spotlight-bg)]',
      spotlightBorder: 'border-[var(--planner-spotlight-border)]',
      spotlightIdentityBg: 'bg-[var(--planner-spotlight-identity-bg)]',
      spotlightSeparator: 'border-[var(--planner-spotlight-separator)]',
      cardBg: 'bg-[var(--planner-card-bg)]',
      cardBorder: 'border-[var(--planner-card-border)]',
      cardText: 'text-[var(--planner-card-text)]',
      nextTaskText: 'text-[var(--planner-next-task-text)]',
      progressRingBase: 'text-[var(--planner-progress-ring-base)]',
      progressRingFill: 'text-[var(--planner-progress-ring-fill)]',
    },
    hierarchy: {
      section: 'flex items-center gap-2 sm:gap-3 group mb-2 bg-[var(--section-bg)] p-1 sm:pl-1.5 sm:pr-5 rounded-[var(--radius-section)] border border-[var(--section-border)] transition-all hover:shadow-md',
      sectionTitle: 'text-lg sm:text-xl font-heading tracking-heading text-[var(--section-title)]',
      ...
      subsection: 'my-1 sm:my-2 bg-[var(--subsection-bg)] rounded-[var(--radius-subsection)] border border-[var(--subsection-border)] p-1 sm:pl-1.5 sm:pr-5 transition-all hover:shadow-md relative group/sub',
      subsectionTitle: 'font-heading tracking-heading text-[var(--subsection-title)] text-sm sm:text-lg break-words whitespace-pre-wrap min-w-0 transition-all duration-300',
    ...
    taskGuide: {
      block: 'bg-[var(--modal-section-bg)] border border-[var(--modal-section-border)] rounded-[var(--radius-modal-section)] p-6 shadow-sm',
      blockPrereq: 'bg-[var(--modal-prereq-bg,transparent)] rounded-[var(--radius-modal-section)] border-2 p-6 transition-all shadow-sm border-[var(--modal-prereq-border,var(--prereq-border))]',
      blockPrereqActive: 'bg-[var(--modal-prereq-bg,var(--prereq-bg))] border-[var(--modal-prereq-border,var(--prereq-border))]',
      header: 'text-[10px] font-black uppercase text-[var(--modal-section-title)] tracking-[0.2em] mb-3 block px-2',
      headerPrereq: 'flex items-center gap-2 text-[var(--prereq-text)] mb-4 px-2',
      headerPrereqIcon: 'text-[var(--prereq-icon)]',
      inputMaster: 'w-full bg-[var(--modal-input-bg)] border-2 border-[var(--modal-input-border)] rounded-[var(--radius-modal-input)] p-4 text-sm font-medium focus:border-google-blue outline-none transition-colors text-[var(--text-primary)] resize-y min-h-[6rem] placeholder-gray-400',
      itemPrereq: 'flex items-center justify-between bg-[var(--prereq-item-bg)] px-4 py-3 rounded-[var(--radius-modal-input)] border border-[var(--prereq-border)] shadow-sm group',
    ...
    docExplorer: {
      ...
      card: 'group relative bg-white dark:bg-black/40 border-2 border-transparent hover:border-google-blue rounded-metadata p-4 shadow-sm transition-all cursor-pointer',
    ...
    taskDoneButton: {
      base: 'flex-1 h-12 rounded-task-button font-black uppercase tracking-[0.15em] text-xs flex items-center justify-center gap-2 transition-all duration-300 shadow-[0_10px_25px_rgba(0,0,0,0.3)] active:scale-95 active:shadow-inner border',
    ...
    sidebar: {
      shell: 'border-gray-300 dark:border-gray-700 bg-sidebar-bg flex flex-col shadow-2xl z-40 fixed md:relative right-0 top-[65px] md:top-0 h-[calc(100vh-65px)] md:h-screen transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] overflow-hidden rounded-sidebar',
    ...
    checklist: {
      ...
      containerActive: 'bg-[var(--task-active-bg)] border-google-green ring-4 ring-google-green/30 shadow-lg',
      containerMulti: 'bg-google-red border-google-red text-white shadow-2xl scale-[1.02] z-20 animate-pulse',
      containerInactive: 'bg-[var(--task-bg)] border-[var(--checklist-border)] hover:bg-black/5 dark:hover:bg-white/5',
    ...
    pomodoro: {
      container: 'flex-1 h-10 flex items-center justify-center px-1.5 py-1 rounded-button transition-all shadow-sm min-w-0 bg-white/10',
      button: 'w-10 h-10 rounded-button flex items-center justify-center flex-shrink-0 transition-all shadow-sm bg-white/20 border-white/30 text-white',
